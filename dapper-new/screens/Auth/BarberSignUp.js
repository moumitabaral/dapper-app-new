...
export default function BarberSignUp({ route, navigation }) {
  ...
  const pickDocument = async () => {
    try {
      ...
      setData({ ...data, document: result.assets[0] });
  ...
  const send = async () => {
    ...
    formData.append("acn", data.acn);
    formData.append("accountName", data.accountName);
    formData.append("bsb", data.bsb);
    ...
    if (data.document) {
      ...
      const filename = data.document.uri.split("/").pop();
      const match = /\.(\w+)$/.exec(filename);
      const ext = match?.[1];
      const type = match ? `document/${match[1]}` : `document`;
      console.log({uri, type, name: `document.${ext}`})
